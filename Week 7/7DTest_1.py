...
def displayResponse(parsed_response: tuple) -> None:
    response_code, response_message, headers_dict, content = parsed_response
    ...
    if response_code == '200':
        print("\nHTML Content:")
        # Check if the content is binary (e.g., an image) and don't try to decode it
        if content[:4] == b'\xff\xd8\xff\xe0' or content[:4] == b'\xff\xd8\xff\xe1':
            print("Binary Content (Likely an Image)")
        else:
